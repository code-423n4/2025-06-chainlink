...
interface IERC4626 is IERC20 {
  ...
  /// @notice Returns the address of the underlying token used for the Vault for accounting,
  ...
  /// @notice Returns the amount of shares that the Vault would exchange for the amount of assets
  ...
  /// reflect the
  /// “average-user’s” price-per-share, meaning what the average user should expect to see
  /// when exchanging to and
  /// from.
  function convertToShares(
    uint256 assets
  ) external view returns (uint256 shares);

  /// @notice Returns the amount of assets that the Vault would exchange for the amount of shares
  /// provided, in an ideal
  /// scenario where all the conditions are met.
  /// @dev
  /// - MUST NOT be inclusive of any fees that are charged against assets in the Vault.
  ...
  /// @notice Returns the maximum amount of the underlying asset that can be deposited into the
  ...
  /// @notice Allows an on-chain or off-chain user to simulate the effects of their deposit at the
  /// current block, given
  /// current on-chain conditions.
  /// @dev
  /// - MUST return as close to and no more than the exact amount of Vault shares that would be
  /// minted in a deposit
  ///   call in the same transaction. I.e. deposit should return the same or more shares as
  /// previewDeposit if called
  ...
  /// - MUST NOT revert.
  ///
  /// NOTE: any unfavorable discrepancy between convertToShares and previewDeposit SHOULD be
  /// considered slippage in
  /// share price or some other type of condition, meaning the depositor will lose assets by
  /// depositing.
  function previewDeposit(
    uint256 assets
  ) external view returns (uint256 shares);

  /// @notice Mints shares Vault shares to receiver by depositing exactly amount of underlying
  /// tokens.
  /// @dev
  ...
  /// @notice Allows an on-chain or off-chain user to simulate the effects of their mint at the
  /// current block, given
  /// current on-chain conditions.
  /// @dev
  /// - MUST return as close to and no fewer than the exact amount of assets that would be deposited
  /// in a mint call
  ///   in the same transaction. I.e. mint should return the same or fewer assets as previewMint if
  /// called in the
  ///   same transaction.
  ...
  /// @notice Mints exactly shares Vault shares to receiver by depositing amount of underlying
  /// tokens.
  /// @dev
  /// - MUST emit the Deposit event.
  /// - MAY support an additional flow in which the underlying tokens are owned by the Vault
  /// contract before the mint
  ...
  /// @notice Returns the maximum amount of the underlying asset that can be withdrawn from the
  ...
  /// @notice Allows an on-chain or off-chain user to simulate the effects of their withdrawal at
  /// the current block,
  /// given current on-chain conditions.
  /// @dev
  /// - MUST return as close to and no fewer than the exact amount of Vault shares that would be
  /// burned in a withdraw
  ///   call in the same transaction. I.e. withdraw should return the same or fewer shares as
  /// previewWithdraw if
  ///   called
  ///   in the same transaction.
  ...
  /// - MUST NOT revert.
  ///
  /// NOTE: any unfavorable discrepancy between convertToShares and previewWithdraw SHOULD be
  /// considered slippage in
  /// share price or some other type of condition, meaning the depositor will lose assets by
  /// depositing.
  ...
  /// @notice Returns the maximum amount of Vault shares that can be redeemed from the owner balance
  /// in the Vault,
  ...
  /// @notice Allows an on-chain or off-chain user to simulate the effects of their redeemption at
  /// the current block,
  /// given current on-chain conditions.
  /// @dev
  /// - MUST return as close to and no more than the exact amount of assets that would be withdrawn
  /// in a redeem call
  ///   in the same transaction. I.e. redeem should return the same or more assets as previewRedeem
  /// if called in the
  ///   same transaction.
  ...
  /// - MUST NOT revert.
  ///
  /// NOTE: any unfavorable discrepancy between convertToAssets and previewRedeem SHOULD be
  /// considered slippage in
  /// share price or some other type of condition, meaning the depositor will lose assets by
  /// redeeming.
  ...
  ///   not having enough shares, etc).
  ///
  /// NOTE: some implementations will require pre-requesting to the Vault before a withdrawal may be
  /// performed.
  /// Those methods should be performed separately.
  function redeem(
    uint256 shares,
