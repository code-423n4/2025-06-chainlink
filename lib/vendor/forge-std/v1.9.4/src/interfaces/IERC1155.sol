...
interface IERC1155 is IERC165 {
  /// @dev
  /// - Either `TransferSingle` or `TransferBatch` MUST emit when tokens are transferred, including
  /// zero value transfers as well as minting or burning (see "Safe Transfer Rules" section of the
  /// standard).
  /// - The `_operator` argument MUST be the address of an account/contract that is approved to make
  /// the transfer (SHOULD be msg.sender).
  /// - The `_from` argument MUST be the address of the holder whose balance is decreased.
  /// - The `_to` argument MUST be the address of the recipient whose balance is increased.
  /// - The `_id` argument MUST be the token type being transferred.
  /// - The `_value` argument MUST be the number of tokens the holder balance is decreased by and
  /// match what the recipient balance is increased by.
  ...
  /// - MUST emit the `TransferSingle` event to reflect the balance change (see "Safe Transfer
  ...
  /// - After the above conditions for the transfer(s) in the batch are met, this function MUST
  /// check if `_to` is a smart contract (e.g. code size > 0). If so, it MUST call the relevant
  /// `ERC1155TokenReceiver` hook(s) on `_to` and act appropriately (see "Safe Transfer Rules"
  ...
  /// @return The _owner's balance of the token types requested (i.e. balance for each (owner, id)
  /// pair)
  ...
  /// @notice Enable or disable approval for a third party ("operator") to manage all of the
  /// caller's tokens.
