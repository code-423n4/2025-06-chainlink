...
library RegistryStorage {
  /// @dev Standardizes `from` storage flags to prevent double-writes in the delegation in/outbox if
  /// the same delegation is revoked and rewritten
  ...
  uint256 internal constant POSITIONS_FIRST_PACKED = 0; //  | 4 bytes empty | first 8 bytes of
    // contract address | 20 bytes of from address |
  uint256 internal constant POSITIONS_SECOND_PACKED = 1; // |        last 12 bytes of contract
    // address          | 20 bytes of to address   |
  ...
  /**
   * @notice Helper function that packs from, to, and contract_ address to into the two slot
   * configuration
   ...
   * @dev Will not revert if `from`, `to`, and `contract_` are > uint160, any inputs with dirty bits
   * outside the last 20 bytes will be cleaned
   */
  function packAddresses(
    ...
    assembly {
      firstPacked :=
        or(shl(64, and(contract_, CLEAN_FIRST8_BYTES_ADDRESS)), and(from, CLEAN_ADDRESS))
      secondPacked := or(shl(160, contract_), and(to, CLEAN_ADDRESS))
    }
  }

  /**
   * @notice Helper function that unpacks from, to, and contract_ address inside the firstPacked
   * secondPacked storage configuration
  ...
  function unpackAddresses(
    ...
    assembly {
      ...
      contract_ :=
        or(shr(64, and(firstPacked, CLEAN_PACKED8_BYTES_ADDRESS)), shr(160, secondPacked))
    }
  }

  /**
   * @notice Helper function that can unpack the from or to address from their respective packed
