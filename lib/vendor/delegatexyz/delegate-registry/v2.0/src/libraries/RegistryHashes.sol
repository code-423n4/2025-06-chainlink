...
library RegistryHashes {
  /// @dev Used to delete everything but the last byte of a 32 byte word with and(word,
  /// EXTRACT_LAST_BYTE)
  ...
  /// @dev Constant for the location of the delegations array in the delegate registry, defined to
  /// be zero
   ...
   * @dev Storage keys further down the array can be obtained by adding computedLocation with the
   * element position
   * @dev Follows the solidity storage location encoding for a mapping(bytes32 => fixedArray) at the
   * position of the delegationSlot
   * @param inputHash The bytehash to decode the type from
  ...
  function location(
    ...
    assembly ("memory-safe") {
      ...
      computedLocation := keccak256(0, 64) // Run keccak256 over bytes in scratch space to obtain
        // the storage key
    }
  }
   ...
   * @dev Equivalent to `keccak256(abi.encodePacked(rights, from, to))` then left-shift by 1 byte
   * and write the delegation type to the cleaned last byte
   * @dev Will not revert if `from` or `to` are > uint160, any input larger than uint160 for `from`
   * and `to` will be cleaned to its lower 20 bytes
  ...
  function allHash(address from, bytes32 rights, address to) internal pure returns (bytes32 hash) {
    assembly ("memory-safe") {
      ...
      mstore(ptr, rights)
      hash := or(shl(8, keccak256(ptr, 72)), ALL_TYPE) // Keccak-hashes the packed encoding,
        // left-shifts by one byte, then writes type to the lowest-order byte
    }
  }
   ...
   * @param rights The rights specified by the delegation
   * @param to The address receiving the delegation
   * @return computedLocation The storage location of the all delegation with those parameters in
  ...
  function allLocation(
    ...
    assembly ("memory-safe") {
      ...
      mstore(add(ptr, 20), from)
      mstore(ptr, rights)
      mstore(0, or(shl(8, keccak256(ptr, 72)), ALL_TYPE)) // Computes `allHash`, then stores the
        // result in scratch space
      mstore(32, DELEGATION_SLOT)
      computedLocation := keccak256(0, 64) // Runs keccak over the scratch space to obtain the
        // storage key
    }
  }
   ...
   * @dev Equivalent to keccak256(abi.encodePacked(rights, from, to, contract_)) left-shifted by 1
   * then last byte overwritten with CONTRACT_TYPE
   * @dev Will not revert if `from`, `to` or `contract_` are > uint160, these inputs will be cleaned
   * to their lower 20 bytes
   * @param from The address making the delegation
   * @param rights The rights specified by the delegation
   * @param to The address receiving the delegation
  ...
  function contractHash(
    ...
    assembly ("memory-safe") {
      ...
      mstore(add(ptr, 20), from)
      mstore(ptr, rights)
      hash := or(shl(8, keccak256(ptr, 92)), CONTRACT_TYPE) // Keccak-hashes the packed encoding,
        // left-shifts by one byte, then writes type to the lowest-order byte
    }
  }
   ...
   * @param to The address receiving the delegation
   * @param contract_ The address of the contract specified by the delegation
   * @return computedLocation The storage location of the contract delegation with those parameters
   * in the delegations mapping
   */
  function contractLocation(
    ...
    assembly ("memory-safe") {
      ...
      mstore(add(ptr, 20), from)
      mstore(ptr, rights)
      mstore(0, or(shl(8, keccak256(ptr, 92)), CONTRACT_TYPE)) // Computes `contractHash`, then
        // stores the result in scratch space
      mstore(32, DELEGATION_SLOT)
      computedLocation := keccak256(0, 64) // Runs keccak over the scratch space to obtain the
        // storage key
    }
   ...
   * @dev Equivalent to `keccak256(abi.encodePacked(rights, from, to, contract_, tokenId))
   * left-shifted by 1 then last byte overwritten with ERC721_TYPE
   * @dev Will not revert if `from`, `to` or `contract_` are > uint160, these inputs will be cleaned
   * to their lower 20 bytes
   * @param from The address making the delegation
   * @param rights The rights specified by the delegation
  ...
  function erc721Hash(
    ...
    assembly ("memory-safe") {
      ...
      mstore(add(ptr, 20), from)
      mstore(ptr, rights)
      hash := or(shl(8, keccak256(ptr, 124)), ERC721_TYPE) // Keccak-hashes the packed encoding,
        // left-shifts by one byte, then writes type to the lowest-order byte
    }
  }
   ...
   * @return computedLocation The storage location of the ERC721 delegation with those parameters in
   * the delegations mapping
   */
  function erc721Location(
    ...
    assembly ("memory-safe") {
      ...
      mstore(add(ptr, 20), from)
      mstore(ptr, rights)
      mstore(0, or(shl(8, keccak256(ptr, 124)), ERC721_TYPE)) // Computes erc721Hash, then stores
        // the result in scratch space
      mstore(32, DELEGATION_SLOT)
      computedLocation := keccak256(0, 64) // Runs keccak256 over the scratch space to obtain the
        // storage key
    }
  }
   ...
   * @dev Equivalent to `keccak256(abi.encodePacked(rights, from, to, contract_))` with the last
   * byte overwritten with ERC20_TYPE
   * @dev Will not revert if `from`, `to` or `contract_` are > uint160, these inputs will be cleaned
   * to their lower 20 bytes
   * @param from The address making the delegation
   * @param rights The rights specified by the delegation
  ...
  function erc20Hash(
    ...
    assembly ("memory-safe") {
      ...
      mstore(add(ptr, 20), from)
      mstore(ptr, rights)
      hash := or(shl(8, keccak256(ptr, 92)), ERC20_TYPE) // Keccak-hashes the packed encoding,
        // left-shifts by one byte, then writes type to the lowest-order byte
    }
  }
   ...
   * @param to The address receiving the delegation
   * @param contract_ The address of the ERC20 token contract
   * @return computedLocation The storage location of the ERC20 delegation with those parameters in
   * the delegations mapping
   */
  function erc20Location(
    ...
    assembly ("memory-safe") {
      ...
      mstore(add(ptr, 20), from)
      mstore(ptr, rights)
      mstore(0, or(shl(8, keccak256(ptr, 92)), ERC20_TYPE)) // Computes erc20Hash, then stores the
        // result in scratch space
      mstore(32, DELEGATION_SLOT)
      computedLocation := keccak256(0, 64) // Runs keccak over the scratch space to obtain the
        // storage key
   ...
   * @dev Equivalent to keccak256(abi.encodePacked(rights, from, to, contract_, tokenId))
   * left-shifted with the last byte overwritten with ERC1155_TYPE
   * @dev Will not revert if `from`, `to` or `contract_` are > uint160, these inputs will be cleaned
   * to their lower 20 bytes
   * @param from The address making the delegation
   * @param rights The rights specified by the delegation
  ...
  function erc1155Hash(
    ...
    assembly ("memory-safe") {
      ...
      mstore(add(ptr, 20), from)
      mstore(ptr, rights)
      hash := or(shl(8, keccak256(ptr, 124)), ERC1155_TYPE) // Keccak-hashes the packed encoding,
        // left-shifts by one byte, then writes type to the lowest-order byte
    }
  }
   ...
   * @param tokenId The id of the ERC1155 token
   * @param contract_ The address of the ERC1155 token contract
   * @return computedLocation The storage location of the ERC1155 delegation with those parameters
   * in the delegations mapping
   */
  function erc1155Location(
    ...
    assembly ("memory-safe") {
      ...
      mstore(add(ptr, 20), from)
      mstore(ptr, rights)
      mstore(0, or(shl(8, keccak256(ptr, 124)), ERC1155_TYPE)) // Computes erc1155Hash, then stores
        // the result in scratch space
      mstore(32, DELEGATION_SLOT)
      computedLocation := keccak256(0, 64) // Runs keccak over the scratch space to obtain the
        // storage key
