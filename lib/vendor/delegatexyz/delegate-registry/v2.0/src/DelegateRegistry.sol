...
contract DelegateRegistry is IDelegateRegistry {
  /// @dev Only this mapping should be used to verify delegations; the other mapping arrays are for
  /// enumerations
  ...
  function sweep() external {
    assembly ("memory-safe") {
      // This hardcoded address is a CREATE2 factory counterfactual smart contract wallet that will
      // always accept native token transfers
      let result :=
        call(gas(), 0x000000dE1E80ea5a234FB5488fee2584251BC7e8, selfbalance(), 0, 0, 0, 0)
    }
  }
  ...
  function checkDelegateForAll(
    ...
    if (!_invalidFrom(from)) {
      ...
      if (!Ops.or(rights == "", valid)) {
        valid = _validateFrom(Hashes.allLocation(from, rights, to), from);
      }
    }
    assembly ("memory-safe") {
      // Only first 32 bytes of scratch space is accessed
      mstore(0, iszero(iszero(valid))) // Compiler cleans dirty booleans on the stack to 1, so do
        // the same here
    ...
    if (!_invalidFrom(from)) {
      ...
      if (!Ops.or(rights == "", amount == type(uint256).max)) {
        uint256 rightsBalance = (
          ...
          ? type(uint256).max
          : _loadDelegationUint(
            Hashes.erc20Location(from, rights, to, contract_), Storage.POSITIONS_AMOUNT
          );
  ...
  function checkDelegateForERC1155(
    ...
    if (!_invalidFrom(from)) {
      amount = (
        ...
        ? type(uint256).max
        : _loadDelegationUint(
          Hashes.erc1155Location(from, "", to, tokenId, contract_), Storage.POSITIONS_AMOUNT
        );
      if (!Ops.or(rights == "", amount == type(uint256).max)) {
        uint256 rightsBalance = (
          _validateFrom(Hashes.allLocation(from, rights, to), from)
          ...
          ? type(uint256).max
          : _loadDelegationUint(
            Hashes.erc1155Location(from, rights, to, tokenId, contract_), Storage.POSITIONS_AMOUNT
          );
        amount = Ops.max(rightsBalance, amount);
      }
    }
  ...
  /// @dev Helper function to push new delegation hashes to the incoming and outgoing hashes
  /// mappings
  ...
  /// @dev Helper function that writes addresses according to the packing rule for delegation
  /// storage
  ...
  /// @dev Helper function that takes an array of delegation hashes and returns an array of
  /// Delegation structs with their onchain information
  ...
  /// @dev Helper function that takes an array of delegation hashes and returns an array of valid
  /// delegation hashes
  ...
  /// @dev Helper function that loads delegation data from a particular array position and returns
  /// as bytes32
  ...
  // @dev Helper function that loads the from address from storage according to the packing rule for
  // delegation storage
  function _loadFrom(
    bytes32 location
  ...
  /// @dev Helper function that loads the address for the delegation according to the packing rule
  /// for delegation storage
