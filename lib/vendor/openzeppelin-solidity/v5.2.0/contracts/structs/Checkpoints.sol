 ...
 *
 * To create a history of checkpoints define a variable type `Checkpoints.Trace*` in your contract,
 * and store a new
...
library Checkpoints {
   ...
   *
   * IMPORTANT: Never accept `key` as a user input, since an arbitrary `type(uint32).max` key set
   * will disable the
   * library.
   */
  function push(
  ...
  /**
   * @dev Return the index of the first (oldest) checkpoint with key strictly bigger than the search
   * key, or `high`
  ...
  /**
   * @dev Access an element of the array without performing bounds check. The position is assumed to
   * be within bounds.
   */
   ...
   * Returns previous value and new value.
   *
   * IMPORTANT: Never accept `key` as a user input, since an arbitrary `type(uint48).max` key set
   * will disable the
   * library.
   */
  function push(
   ...
   * Returns previous value and new value.
   *
   * IMPORTANT: Never accept `key` as a user input, since an arbitrary `type(uint96).max` key set
   * will disable the
   * library.
   */
  function push(
