 ...
 *
 * This data structure allows for pushing elements to it, and when its length exceeds the specified
 * fixed size,
 ...
 * - reset ({clear}): O(1)
 *
 * * The struct is called `Bytes32CircularBuffer`. Other types can be cast to and from `bytes32`.
 * This data structure
...
library CircularBuffer {
  ...
  /**
   * @dev Counts the number of items that have been pushed to the buffer. The residuo modulo
   * _data.length indicates
   ...
   *
   * If the CircularBuffer was already setup and used, calling that function again will reset it to
   * a blank state.
   *
   * NOTE: The size of the buffer will affect the execution of {includes} function, as it has a
   * complexity of O(N).
   ...
   */
  function clear(
    Bytes32CircularBuffer storage self
  ) internal {
    self._count = 0;
  }

  /**
   * @dev Push a new value to the buffer. If the buffer is already full, the new value replaces the
   * oldest value in
   * the buffer.
   */
  ...
  /**
   * @dev Number of values currently in the buffer. This value is 0 for an empty buffer, and cannot
   * exceed the size of
   * the buffer.
   */
  function count(
   ...
   *
   * Reverts with {Panic-ARRAY_OUT_OF_BOUNDS} if trying to access an element that was not pushed, or
   * that was
