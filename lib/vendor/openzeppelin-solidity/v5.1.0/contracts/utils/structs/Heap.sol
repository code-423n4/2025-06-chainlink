...
/**
 * @dev Library for managing https://en.wikipedia.org/wiki/Binary_heap[binary heap] that can be used
 * as
 * https://en.wikipedia.org/wiki/Priority_queue[priority queue].
 *
 * Heaps are represented as a tree of values where the first element (index 0) is the root, and
 * where the node at
 * index i is the child of the node at index (i-1)/2 and the parent of nodes at index 2*i+1 and
 * 2*i+2. Each node
 * stores an element of the heap.
 *
 * The structure is ordered so that each node is bigger than its parent. An immediate consequence is
 * that the
 * highest priority value is the one at the root. This value can be looked up in constant time
 * (O(1)) at
 * `heap.tree[0]`
 *
 * The structure is designed to perform the following operations with the corresponding
 * complexities:
 *
 ...
 *
 * IMPORTANT: This library allows for the use of custom comparator functions. Given that
 * manipulating
 * memory can lead to unexpected behavior. Consider verifying that the comparator does not
 * manipulate
...
library Heap {
   ...
   *
   * NOTE: All inserting and removal from a heap should always be done using the same comparator.
   * Mixing comparator
  ...
  /**
   * @dev Return the root element for the heap, and replace it with a new value, using the default
   * comparator.
  ...
  /**
   * @dev Return the root element for the heap, and replace it with a new value, using the provided
   * comparator.
   * This is equivalent to using {pop} and {insert}, but requires only one rebalancing operation.
   *
   * NOTE: All inserting and removal from a heap should always be done using the same comparator.
  ...
  /**
   * @dev Perform heap maintenance on `self`, starting at `index` (with the `value`), using `comp`
   * as a
   * comparator, and moving toward the leaves of the underlying tree.
   *
   * NOTE: This is a private function that is called in a trusted context with already cached
   * parameters. `size`
   * and `value` could be extracted from `self` and `index`, but that would require redundant
   * storage read. These
  ...
  function _siftDown(
    ...
    unchecked {
      // Check if there is a risk of overflow when computing the indices of the child nodes. If that
      // is the case,
   ...
   * NOTE: This is a private function that is called in a trusted context with already cached
   * parameters. `value`
   * could be extracted from `self` and `index`, but that would require redundant storage read. These
   * parameters are not
