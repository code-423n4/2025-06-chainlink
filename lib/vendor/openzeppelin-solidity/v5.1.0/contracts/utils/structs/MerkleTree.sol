 ...
 *
 * Each tree is a complete binary tree with the ability to sequentially insert leaves, changing them
 * from a zero to a
 * non-zero value and updating its root. This structure allows inserting commitments (or other
 * entries) that are not
 * stored, but can be proven to be part of the tree at a later time if the root is kept. See
 * {MerkleProof}.
 *
 * A tree is defined by the following parameters:
 *
 * * Depth: The number of levels in the tree, it also defines the maximum number of leaves as
 * 2**depth.
 * * Zero value: The value that represents an empty leaf. Used to avoid regular zero values to be
 * part of the tree.
 * * Hashing function: A cryptographic hash function used to produce internal nodes. Defaults to
 * {Hashes-commutativeKeccak256}.
 *
 * NOTE: Building trees using non-commutative hashing functions (i.e. `H(a, b) != H(b, a)`) is
 * supported. However,
 * proving the inclusion of a leaf in such trees is not possible with the {MerkleProof} library
...
library MerkleTree {
   ...
   *
   * The `sides` and `zero` arrays are set to have a length equal to the depth of the tree during
   * setup.
   *
   * Struct members have an underscore prefix indicating that they are "private" and should not be
   * read or written to
   * directly. Use the functions provided below instead. Modifying the struct manually may violate
   * assumptions and
   * lead to unexpected behavior.
   *
   * NOTE: The `root` and the updates history is not stored within the tree. Consider using a
   * secondary structure to
   * store a list of historical roots from the values returned from {setup} and {push} (e.g. a
   * mapping, {BitMaps} or
   * {Checkpoints}).
   *
   * WARNING: Updating any of the tree's parameters after the first insertion will result in a
   * corrupted tree.
   */
   ...
   *
   * Calling this function on MerkleTree that was already setup and used will reset it to a blank
   * state.
   *
   * Once a tree is setup, any push to it must use the same hashing function. This means that values
   * should be pushed to it using the default
   * {xref-MerkleTree-push-struct-MerkleTree-Bytes32PushTree-bytes32-}[push] function.
   *
   * IMPORTANT: The zero value should be carefully chosen since it will be stored in the tree
   * representing
  ...
  /**
   * @dev Same as {xref-MerkleTree-setup-struct-MerkleTree-Bytes32PushTree-uint8-bytes32-}[setup],
   * but allows to specify a custom hashing function.
   *
   * Once a tree is setup, any push to it must use the same hashing function. This means that values
   * should be pushed to it using the custom push function, which should be the same one as used
   * during the setup.
   *
   ...
   *
   * NOTE: Consider verifying that the hashing function does not manipulate the memory state
   * directly and that it
  ...
  /**
   * @dev Insert a new leaf in the tree, and compute the new root. Returns the position of the
   * inserted leaf in the
   * tree, and the resulting root.
   *
   * Hashing the leaf before calling this function is recommended as a protection against
   * second pre-image attacks.
   *
   * This variant uses {Hashes-commutativeKeccak256} to hash internal nodes. It should only be used
   * on merkle trees
   * that were setup using the same (default) hashing function (i.e. by calling
   * {xref-MerkleTree-setup-struct-MerkleTree-Bytes32PushTree-uint8-bytes32-}[the default setup]
   * function).
   */
  function push(
    Bytes32PushTree storage self,
   ...
   * second pre-image attacks.
   *
   * This variant uses a custom hashing function to hash internal nodes. It should only be called
   * with the same
