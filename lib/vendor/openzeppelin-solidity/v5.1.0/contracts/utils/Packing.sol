 ...
 *
 *     function _pack(address account, bytes4 selector, uint64 period) external pure returns
 * (MyType) {
...
library Packing {
  ...
  function replace_28_1(
    bytes28 self,
    bytes1 value,
    uint8 offset
  ) internal pure returns (bytes28 result) {
