 ...
 *
 * The secp256r1 curve (also known as P256) is a NIST standard curve with wide support in modern
 * devices
 * and cryptographic standards. Some notable examples include Apple's Secure Enclave and Android's
 * Keystore
 * as well as authentication protocols like FIDO2.
 *
 * Based on the original
 * https://github.com/itsobvioustech/aa-passkeys-wallet/blob/d3d423f28a4d8dfcb203c7fa0c47f42592a7378e/src/Secp256r1.sol[implementation
 * of itsobvioustech] (GNU General Public License v3.0).
 * Heavily inspired in
 * https://github.com/maxrobot/elliptic-solidity/blob/c4bb1b6e8ae89534d8db3a6b3a6b52219100520f/contracts/Secp256r1.sol[maxrobot]
 * and
 * https://github.com/tdrerup/elliptic-curve-solidity/blob/59a9c25957d4d190eff53b6610731d81a077a15e/contracts/curves/EllipticCurve.sol[tdrerup]
 * implementations.
 *
...
library P256 {
  ...
  /**
   * @dev Verifies a secp256r1 signature using the RIP-7212 precompile and falls back to the
   * Solidity implementation
   * if the precompile is not available. This version should work on all chains, but requires the
   * deployment of more
   * bytecode.
   *
   ...
   *
   * IMPORTANT: This function disallows signatures where the `s` value is above `N/2` to prevent
   * malleability.
  ...
  function _tryVerifyNative(
    ...
    return (success && returndata.length == 0x20)
      ? (abi.decode(returndata, (bool)), true)
      : (false, false);
  }
  ...
  function recovery(
    ...
    uint256 ry2 = addmod(mulmod(addmod(mulmod(rx, rx, p), A, p), rx, p), B, p); // weierstrass
      // equation y² = x³ + a.x + b
  ...
  function isValidPublicKey(bytes32 x, bytes32 y) internal pure returns (bool result) {
    assembly ("memory-safe") {
      ...
      let rhs := addmod(mulmod(addmod(mulmod(x, x, p), A, p), x, p), B, p) // ((x^2 + a) * x) + b =
        // x^3 + ax + b
      result := and(and(lt(x, p), lt(y, p)), eq(lhs, rhs)) // Should conform with the Weierstrass
        // equation
    }
  }
   ...
   * In particular, this checks that `s` is in the "lower-range", making the signature
   * non-malleable.
   */
   ...
   * Note that:
   *
   * - `addition-add-1998-cmo-2` doesn't support identical input points. This version is modified to
   * use
   * the `h` and `r` values computed by `addition-add-1998-cmo-2` to detect identical inputs, and
   * fallback to
  ...
  function _jAdd(
    ...
    assembly ("memory-safe") {
      ...
      case 1 {
        ...
        let yy := mulmod(y, y, p)
        let zz := mulmod(z, z, p)
        let m := addmod(mulmod(3, mulmod(x, x, p), p), mulmod(A, mulmod(zz, zz, p), p), p) // m =
          // 3*x²+a*z⁴
  ...
  function _jDouble(
    ...
    assembly ("memory-safe") {
      ...
      // y' = m*(s-t)-8*y⁴ = m*(s-x')-8*y⁴
      ry :=
        addmod(mulmod(m, addmod(s, sub(p, rx), p), p), sub(p, mulmod(8, mulmod(yy, yy, p), p)), p)
      // z' = 2*y*z
      rz := mulmod(2, mulmod(y, z, p), p)
    }
  }
  ...
  /**
   * @dev Compute G·u1 + P·u2 using the precomputed points for G and P (see
   * {_preComputeJacobianPoints}).
   *
   * Uses Strauss Shamir trick for EC multiplication
   * https://stackoverflow.com/questions/50993471/ec-scalar-multiplication-with-strauss-shamir-method
   *
   * We optimize this for 2 bits at a time rather than a single bit. The individual points for a
   * single pass are
  ...
  function _jMultShamir(
    ...
    unchecked {
      for (uint256 i = 0; i < 128; ++i) {
        ...
        // Read 2 bits of u1, and 2 bits of u2. Combining the two gives the lookup index in the
        // table.
  ...
  /**
   * @dev Precompute a matrice of useful jacobian points associated with a given P. This can be seen
   * as a 4x4 matrix
   ...
   * └────┴─────────────────────┘
   *
   * Note that `_jAdd` (and thus `_jAddPoint`) does not handle the case where one of the inputs is a
   * point at
   * infinity (z = 0). However, we know that since `N ≡ 1 mod 2` and `N ≡ 1 mod 3`, there is no
   * point P such that
   * 2P = 0 or 3P = 0. This guarantees that g, 2g, 3g, p, 2p, 3p are all non-zero, and that all
   * `_jAddPoint` calls
   * have valid inputs.
   */
  function _preComputeJacobianPoints(
    uint256 px,
    uint256 py
  ) private pure returns (JPoint[16] memory points) {
  ...
  function _jDoublePoint(
    JPoint memory p
  ) private pure returns (JPoint memory) {
