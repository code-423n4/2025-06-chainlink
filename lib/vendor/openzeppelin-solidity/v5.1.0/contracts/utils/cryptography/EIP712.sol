 ...
 *
 * The encoding scheme specified in the EIP requires a domain separator and a hash of the typed
 * structured data, whose
 * encoding is very generic and therefore its implementation in Solidity is not feasible, thus this
 * contract
 * does not implement the encoding itself. Protocols need to implement the type-specific encoding
 ...
 * of the encoding
 * scheme, and the final step of the encoding to obtain the message digest that is then signed via
 * ECDSA
 * ({_hashTypedDataV4}).
 *
 * The implementation of the domain separator was designed to be as efficient as possible while
 * still properly updating
 ...
 *
 * NOTE: In the upgradeable version of this contract, the cached values will correspond to the
 * address, and the domain
 * separator of the implementation contract. This will cause the {_domainSeparatorV4} function to
 * always rebuild the
 * separator from the immutable values, which is cheaper than accessing a cached version in cold
 * storage.
 *
...
abstract contract EIP712 is IERC5267 {
   ...
   * It only reads from storage if necessary (in case the value is too large to fit in a
   * ShortString).
   */
