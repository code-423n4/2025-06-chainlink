...
/**
 * @dev RSA PKCS#1 v1.5 signature verification implementation according to
 * https://datatracker.ietf.org/doc/html/rfc8017[RFC8017].
 *
 * This library supports PKCS#1 v1.5 padding to avoid malleability via chosen plaintext attacks in
 * practical implementations.
 * The padding follows the EMSA-PKCS1-v1_5-ENCODE encoding definition as per section 9.2 of the RFC.
 * This padding makes
 * RSA semantically secure for signing messages.
 *
 * Inspired by
 * https://github.com/adria0/SolRsaVerify/blob/79c6182cabb9102ea69d4a2e996816091d5f1cd1[Adrià
 * Massanet's work] (GNU General Public License v3.0).
 *
...
library RSA {
   ...
   * method described in https://datatracker.ietf.org/doc/html/rfc8017#section-8.2.2[section 8.2.2
   * of RFC8017] with
   * support for explicit or implicit NULL parameters in the DigestInfo (no other optional
   * parameters are supported).
   *
   * IMPORTANT: For security reason, this function requires the signature and modulus to have a
   * length of at least
   * 2048 bits. If you use a smaller key, consider replacing it with a larger, more secure, one.
   *
   * WARNING: This verification algorithm doesn't prevent replayability. If called multiple times
   * with the same
   * digest, public key and (valid signature), it will return true every time. Consider including an
   * onchain nonce
   * or unique identifier in the message to prevent replay attacks.
   *
   * WARNING: This verification algorithm supports any exponent. NIST recommends using `65537` (or
   * higher).
   * That is the default value many libraries use, such as OpenSSL. Developers may choose to reject
   * public keys
  ...
  function pkcs1Sha256(
    ...
    unchecked {
      ...
      if (bytes1(_unsafeReadBytes32(buffer, length - 0x32)) == 0x31) {
        offset = 0x34;
        // 00 (1 byte) | SEQUENCE length (0x31) = 3031 (2 bytes) | SEQUENCE length (0x0d) = 300d (2
        // bytes) | OBJECT_IDENTIFIER length (0x09) = 0609 (2 bytes)
        // SHA256 OID = 608648016503040201 (9 bytes) | NULL = 0500 (2 bytes) (explicit) |
        // OCTET_STRING length (0x20) = 0420 (2 bytes)
        ...
        offset = 0x32;
        // 00 (1 byte) | SEQUENCE length (0x2f) = 302f (2 bytes) | SEQUENCE length (0x0b) = 300b (2
        // bytes) | OBJECT_IDENTIFIER length (0x09) = 0609 (2 bytes)
        // SHA256 OID = 608648016503040201 (9 bytes) | NULL = <implicit> | OCTET_STRING length
        // (0x20) = 0420 (2 bytes)
      ...
      // Length is at least 0x100 and offset is at most 0x34, so this is safe. There is always some
      // padding.
      ...
      // All the other parameters are small enough to fit in a bytes32, so we can check them
      // directly.
  ...
  function _unsafeReadBytes32(
    ...
    // Memory safeness is guaranteed as long as the provided `array` is a Solidity-allocated bytes
    // array
    // and `offset` is within bounds. This is the case for all calls to this private function from
    // {pkcs1Sha256}.
