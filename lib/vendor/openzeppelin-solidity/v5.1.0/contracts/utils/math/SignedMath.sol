...
library SignedMath {
  ...
  function abs(
    ...
    unchecked {
      ...
      // Since `n` is a signed integer, the generated bytecode will use the SAR opcode to perform
      // the right shift,
      ...
      // the mask will either be `bytes32(0)` (if n is positive) or `~bytes32(0)` (if n is
      // negative).
      int256 mask = n >> 255;
